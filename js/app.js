...
var Review = function(review) {
	...
	this.photo = "images/" + review.firstName.toLowerCase() + ".jpg";
	this.author = review.fullName;
	this.location = review.location;
	this.title = review.reviewTitle;
	this.body = review.reviewBody;
	this.rating = review.starRating; 
	this.ratingText = "Rating: " + review.starRating + " out of 5";
	this.reviewsCount = review.reviewsCount;
}
...
var AppViewModel = function(jsonData) {
	...
	// Vars and bindable collections
	this.reviewList = ko.observableArray([]);

    // Populate reviews list
	for (var i = 0; i < reviews.length; i++) {
		var reviewItem = reviews[i];
		self.reviewList.push(new Review(reviewItem));
	};
}
...
var view = {
	isWidgetOpen: false,
	widgetButton: document.getElementById("badge"),
	init: function() {
		$("#content-wrap").click(function(e) {
			if (view.isWidgetOpen && e.target !== view.widgetButton) {
				view.toggleWidget();
			};
		});

		$(view.widgetButton).click(function() {
			view.toggleWidget();
		});
	...
	toggleWidget: function() {
		...
		view.isWidgetOpen = !view.isWidgetOpen;
	},
...
$(function() {
	...
	view.init();
	// Init view model
	ko.applyBindings(new AppViewModel(reviews));
});
